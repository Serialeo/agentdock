...
COPY --chmod=0755 packaging/docker/docker-healthcheck.sh /usr/local/bin/agentdock-healthcheck
COPY core-skills /tmp/agentdock-bundle/core-skills
COPY packaging/build-core-skill-bundle.py /tmp/agentdock-bundle/build-core-skill-bundle.py
...
WORKDIR /src
USER agentdock:agentdock
# Use a disposable HOME so module/cache artifacts created by the browser integration
# test are removed even when Go leaves read-only module directories behind.
RUN set -eu; \
