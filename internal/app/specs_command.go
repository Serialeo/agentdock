...
func commandToolSpecs() []ToolSpec {
	return []ToolSpec{
		...
		{Name: "session_observe", Contract: commandToolContract, Title: "Observe command sessions", Description: "List or inspect command sessions through a read-only session tool.", Annotations: readOnlyToolAnnotations(false), Handler: typedToolHandler("session_observe", func(ctx context.Context, r *Runtime, request toolcommand.SessionObserveRequest) (Result, error) {
			return r.command.Observe(ctx, request)
		})},
		{Name: "session_act", Contract: commandToolContract, Title: "Act on command sessions", Description: "Write to or stop command sessions through a mutating session tool.", Annotations: mutatingToolAnnotations(true, true), Handler: typedToolHandler("session_act", func(ctx context.Context, r *Runtime, request toolcommand.SessionActRequest) (Result, error) {
			return r.command.Act(ctx, request)
		})},
	}
}
